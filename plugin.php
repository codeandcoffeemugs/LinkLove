...
Version: 1.0
Author: John Mangino, Fat Panda
Author URI: http://
Plugin URI: http://fatpandadev.com/wp-linklove
*/
...
class WpLinkLove {
  ...
  const AINT_WE_GOT_FUN = 4;
	...
	const OPTION_SESSION = '_wplinklove_session_key';
  ...
  function init() {
    ...
    if ( is_admin() ) {
     ...
     #
    add_shortcode( 'linklove', array($this, 'linklove_shortcode') );
  ...
  function admin_menu() {
    ...
    //////////////////////////////////////////////////////////////////////////
		...
		if (get_option(self::OPTION_SESSION)) {
			add_settings_field(
				'reset_session',
				'Reset Session',
				array( $this, 'reset_session'),
				__CLASS__,
				'basic'
			);
	    ...
	    add_settings_field( 
	      ...
	      'basic',
	      array(
	        'label_for' => $this->id('username', false)
	      )
	    );
		    ...
		    'basic',
		    array(
		      'label_for' => $this->id('password', false)
		    )
		  );
		}
  ...
  function __call($name, $args) {
    ...
    if ($name === 'basic_settings') {
      ...
      <?php
			if ($key = get_option(self::OPTION_SESSION)) {
				echo "<p>{$key}</p>";
			}
    }
    ...
    if ($name === 'username') {
      ?>
        <input type="text" class="regular-text" id="<?php $this->id($name) ?>" name="<?php $this->field($name) ?>" value="<?php echo esc_attr( $this->setting($name) ) ?>" />
      <?php
			if ($this->setting($name) && !get_option(self::OPTION_SESSION)) {
				?>
					&nbsp; <span class="description" style="color:red">Hey numb-nuts, wrong password.</span>
				<?php
			}
    }
		...
		if ($name === 'password') {
      ?>
        <input type="password" class="regular-text" id="<?php $this->id($name) ?>" name="<?php $this->field($name) ?>" />
      <?php
    }

		if ($name == 'reset_session') {
			?>
				<input type="submit" class="button" name="<?php $this->field($name) ?>" value="Reset Session Now" />
			<?php
		}
  ...
  function sanitize_settings($settings) {
		...
		if ($reset_session) {
			...
			// do some login function
			// make sure the session key is valid
			// store it in an option
			
			update_option(self::OPTION_SESSION, time());
			unset($settings['username']);
			unset($settings['password']);
		}
	
		unset($settings['reset_session']);
